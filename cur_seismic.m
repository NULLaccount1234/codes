...
data = 'seismic';
segyFilePath = './data/seismic.sgy';
[M, SegyTraceHeaders, SegyHeader] = ReadSegy(segyFilePath);
M = reshape(M, [51, 191, 146]);
parameters(1).osr = 0.12;
parameters(1).iter = 100;
parameters(1).deltas = [0.15, 0.16, 0.17];
parameters(1).r =1;
parameters(2).osr = 0.16;
parameters(2).iter = 100;
parameters(2).deltas = [0.15, 0.16, 0.17];
parameters(2).r = 2;
parameters(3).osr = 0.20;
parameters(3).iter = 100;
parameters(3).deltas = [0.15, 0.16, 0.17];
parameters(3).r = 2;
parameters(4).osr = 0.24;
parameters(4).iter = 100;
parameters(4).deltas = [0.15, 0.16, 0.17];
parameters(4).r = 3;
parameters(5).osr = 0.28;
parameters(5).iter = 100;
parameters(5).deltas = [0.15, 0.16, 0.17];
parameters(5).r = 3;
...
disp(results_table)
filename =  sprintf('/mnt/research/huanglx_group/cur_seismic_%d.mat', k);
